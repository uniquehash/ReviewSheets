...
* how do we [add developer metadata to our spreadsheets using GSA](https://developers.google.com/sheets/api/guides/metadata)?
	* developer metadata feature allows you to associate metadata with various entities and locations in a spreadsheet
	* you can query this metadata and use it to look up the objects with which it is associated
	* performable operations
		* associate arbitrary data with various entities and locations in a spreadsheet
			* associate 'totals' with column D
			* associate 'responseId -> 1234' with row 7	
		* look up all locations and data associated with a particular metadata key or attribute
			* return that 'totals' is associated with column D
			* return the set of all rows with 'responseId' metadata and the metadata value associated with it
		* look up all data associated with a particular entity or location
			* return all metadata associated with column D
		* retrieve values in a location by specifying associated metadata
			* return a representation of the values contained in the associated column or row for 'totals'
		* update values in a location by specifying associated metadata
			* update values by indicating a metadata ID instead of updating values by providing a range
	* about developer metadata
		* metadata as tags
			* metadata can be used as tags that name a location in the spreadsheet
				* specifies only a key and a location
			* can be used to semantically bind portions of a spreadsheet to fields in a third-party tool or database
		* metadata as properties
			* metadata can be used to assign properties to sheet locations
			* specifying a key, location, and a value will cause a key-value pair to be created and associated with the location
			* allows you to store and access custom named properties associated with particular areas or data in a spreadsheet
		* project vs document visible metadata
			* there are two metadata visibilities
				* project
					* metadata that is only visible and accessible from the developer project that created it
					* do not think this is secure
				* document
					* metadata will be accessible from any developer project with access to the document
		* uniqueness
			* metadata keys do not have to be unique
			* metadata IDs have to be unique
				* new metadata with ID field unspecified will be be autogenerated
	* reading and writing metadata
		* creating new metadata
			* use the `batchUpdate` method
				* supply a `createDeveloperMetadataRequest`
					* `key`
						* the key to be used to semantically identify the metadata
					* `location`
						* the location in the sheet to associate the metadata with
					* `value`
						* optional, the value to associate with the key
					* `visibility`
						* either project or document determines the type of visibility
					* `id`
						* optional, uniquely identifies the metadata
		* reading a single metadata item
			* use `spreadsheets.metadata.get` specifying
				* unique `spreadsheetId`
				* `metadataId`
		* reading multiple metadata items
			* use `spreadsheets.metadata.search` specifying
				* `DataFilter`
					* matches existing metadata on any combination of properties such as
						* key
						* value
						* location
						* visibility
		* updating metadata
			* use `batchUpdate` with `UpdateDeveloperMetadataRequest` and specify
				* `DataFilter`
					* targets the metadata to be updated
				* `DeveloperMetadata`
					* object containing new values
				* `FieldMask`
					* describes which fields should be updated
		* deleting metadata
			* use `batchUpdate` with `DeleteDeveloperMetadataRequest` specifying
				* `DataFilter`
					* matches the data you want to delete
	* reading and writing values associated with metadata
		* get values by metadata
			* to get cell values by metadata use `spreadsheets.values.batchGetByDataFilter` specifying 
				* `DataFilter`
					* matches metadata 
		* get spreadsheet by metadata
			* when retrieving a spreadsheet you can target the subset of data to be returned
			* use `spreadsheets.getByDataFilter` specifying
				* `DataFilter`
			* can optionally include all the normal grid data if `includeGridData` is set to true
		* updating values by metadata
			* update the value of cells that match specific metadata
			* use `spreadsheets.values.batchUpdateByDataFilter` specifying
				* `DataFilter`
	* metadata storage limits
		* spreadsheet
			* can store 30,000
		* each sheet in the spreadsheet 
			* can store additional 30,000
		* each character in the key and value attributes count towards this limit
